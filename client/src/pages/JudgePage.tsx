...
export default function JudgePage() {
  ...
  const [penaltyType, setPenaltyType] = useState<
    "wrong_answer" | "false_start" | "rule_violation" | "custom"
  >("wrong_answer");
  ...
  const urls = useMemo(() => {
    ...
    return teams
      .filter((t) => t.enabled)
      .map((t) => ({ id: t.id, name: t.name, url: `${base}/team/${t.id}` }));
  }, [teams]);

  const handleExportCSV = () => {
    window.open(`${window.location.origin}/api/logs/csv`, "_blank");
  };
  const handleExportJSON = () => {
    window.open(`${window.location.origin}/api/logs/json`, "_blank");
  };
  ...
  return (
    <div className="flex flex-col h-full bg-bg overflow-y-auto">
      <header className="sticky top-0 z-10 bg-bg border-b border-border px-4 py-3 md:px-6 md:py-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="text-[0.65rem] font-semibold uppercase tracking-wider text-text-muted">
              Dashboard Juri{urlRole ? ` (${urlRole})` : ""}
            </div>
          </div>
        ...
        <div className="mt-1">
          <NetworkIndicator quality={quality} ping={ping} />
        </div>
        ...
        {emergencyState !== "none" && (
          <div className="flex items-center gap-3 mt-3 px-4 py-2.5 rounded-lg bg-orange/10 border border-orange font-bold text-orange">
            ...
            <span className="flex-1">
              EMERGENCY: {emergencyState.toUpperCase()}
            </span>
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          <Button
            variant="success"
            onClick={startRound}
            disabled={isReady || emergencyState !== "none"}
          >
          ...
          <Button
            variant={questionReading ? "destructive" : "outline"}
            onClick={() => setQuestionReading(!questionReading)}
          >
            <BookOpen className="w-4 h-4" />{" "}
            {questionReading ? "End Reading" : "Reading"}
          </Button>
        </div>
        ...
        <div className="flex gap-1.5 mt-2 flex-wrap">
          ...
          <Button
            variant="outline"
            size="xs"
            onClick={() => setShowCompSettings(!showCompSettings)}
          >
      ...
      <main className="p-4 md:p-6 flex flex-col gap-4">
            ...
            <CardContent>
              <div
                className={`text-2xl font-extrabold tracking-wider ${
                  stateLabel === "READY" || stateLabel === "BUZZER_OPEN"
                    ? "text-success"
                    : stateLabel === "LOCKED"
                      ? "text-danger"
                      : "text-text-muted"
                ...
                <div className="flex flex-wrap gap-1.5">
                  {connectedTeams.map((t) => {
                    ...
                    return (
                      <Badge
                        key={t}
                        variant={winner === t ? "warning" : "default"}
                      >
                        <div
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {urls.map((u) => (
                  <QrCodeView key={u.id} url={u.url} label={u.name} />
                ))}
              </div>
            </CardContent>
          </Card>
        ...
        <Panel open={showPenalty} onClose={() => setShowPenalty(false)}>
          <CardHeader>
            <CardTitle className="text-danger">Apply Penalty</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {compSettings && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <Select
                    value={compSettings.falseStartAction}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        falseStartAction: e.target.value,
                      })
                    }
                    className="w-full"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={compSettings.penaltyConfig.wrongAnswer}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        penaltyConfig: {
                          ...compSettings.penaltyConfig,
                          wrongAnswer: Number(e.target.value),
                        },
                      })
                    }
                  />
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={compSettings.penaltyConfig.falseStart}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        penaltyConfig: {
                          ...compSettings.penaltyConfig,
                          falseStart: Number(e.target.value),
                        },
                      })
                    }
                  />
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={compSettings.penaltyConfig.ruleViolation}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        penaltyConfig: {
                          ...compSettings.penaltyConfig,
                          ruleViolation: Number(e.target.value),
                        },
                      })
                    }
                  />
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={compSettings.questionReadingDuration}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        questionReadingDuration: Number(e.target.value),
                      })
                    }
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={compSettings.rebuttalLockDuration}
                    onChange={(e) =>
                      updateCompetitionSettings({
                        rebuttalLockDuration: Number(e.target.value),
                      })
                    }
                  />
                </div>
            ...
            <div className="space-y-3">
              {teams.map((t) => {
                ...
                return (
                  <div
                    key={t.id}
                    className="flex items-center gap-3 p-2.5 rounded-lg border border-border flex-wrap"
                  >
                    <div
                    ...
                    <Input
                      ...
                      onBlur={(e) =>
                        updateTeamProfile(t.id, { institution: e.target.value })
                      }
                      className="h-8 flex-1 min-w-[120px]"
                    />
                    <Input
                      placeholder="Members (comma separated)"
                      defaultValue={profile?.members?.join(", ") ?? ""}
                      onBlur={(e) =>
                        updateTeamProfile(t.id, {
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {bracket.matches.map((m) => (
                    <div
                      key={m.id}
                      className="flex items-center gap-3 p-2.5 rounded-lg bg-bg-subtle text-sm"
                    >
                      <span className="font-semibold">
                        {m.team1Id
                          ? (teams.find((t) => t.id === m.team1Id)?.name ??
                            m.team1Id)
                          : "TBD"}
                      </span>
                      ...
                      <span className="font-semibold">
                        {m.team2Id
                          ? (teams.find((t) => t.id === m.team2Id)?.name ??
                            m.team2Id)
                          : "TBD"}
                      </span>
                      {m.winnerId && (
                      ...
                      {!m.winnerId && m.team1Id && m.team2Id && (
                        <Select
                          ...
                          onChange={(e) => {
                            if (e.target.value)
                              advanceWinner(m.id, e.target.value);
                          }}
                          value=""
                        >
                          <option value="">Pick winner</option>
                          <option value={m.team1Id}>
                            {teams.find((t) => t.id === m.team1Id)?.name}
                          </option>
                          <option value={m.team2Id}>
                            {teams.find((t) => t.id === m.team2Id)?.name}
                          </option>
                        </Select>
                      )}
              ...
              <div className="text-center py-6">
                ...
                <Button
                  ...
                  onClick={() =>
                    createBracket(
                      "quarter_final",
                      teams.filter((t) => t.enabled).map((t) => t.id),
                    )
                  }
                >
                  Create Bracket
            ...
            {analytics && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    label: "Fastest Team",
                    value: analytics.fastestTeam?.teamName ?? "—",
                    Icon: Zap,
                    color: "text-warning",
                  },
                  {
                    label: "Most Correct",
                    value: analytics.mostCorrect?.teamName ?? "—",
                    Icon: Trophy,
                    color: "text-success",
                  },
                  {
                    label: "Most Active",
                    value: analytics.mostActive?.teamName ?? "—",
                    Icon: Activity,
                    color: "text-purple",
                  },
                  ...
                  <div
                    key={label}
                    className="p-3 rounded-lg bg-bg-subtle text-center"
                  >
